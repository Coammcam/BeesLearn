...
private val vowels = listOf(
    ...
    IPAModel("eɪ", "say", R.raw.but, R.raw.say, R.raw.say),
...
fun IPAExercise(navController: NavController) {
    Column(
        ...
        IPAGrid(symbols = vowels, navController)
        Spacer(modifier = Modifier.height(50.dp))

        SectionTitle(title = "Phụ âm")
        Spacer(modifier = Modifier.height(16.dp))
        IPAGrid(symbols = consonants, navController)
...
@Composable
fun IPAGrid(symbols: List<IPAModel>, navController: NavController) {
    ...
    LazyVerticalGrid(
        ...
        items(symbols.size) { index ->
            val (symbol, example, soundSymbol, soundExample, videoResId) = symbols[index]
            if (videoResId != 0) {
                IPACard(
                    symbol = symbol,
                    example = example,
                    soundSymbol = soundSymbol ?: 0,
                    soundExample = soundExample ?: 0,
...
fun playSound(
    ...
    val resId =
        context.resources.getIdentifier(fileName.removeSuffix(".mp4"), "raw", context.packageName)
    if (resId != 0) {
